...
- 이 인가 코드(code)는 REST API 키로만 access token으로 교환할 수 있어, `/api/kakao-token` (Vercel 서버리스 함수)이 그 교환을 대신합니다. 그 후 `/api/verify-kakao-comment`가 이 access token을 `kapi.kakao.com`에 직접 검증한 뒤, `service_role` 키로만 Supabase에 insert합니다.
