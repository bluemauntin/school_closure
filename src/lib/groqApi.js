  ...
  const messages = [
    {
      role: 'system',
      content:
        '당신은 한국 교육 통계를 분석하는 전문가입니다. 학교 신입생 수 추이 또는 지역 특성을 바탕으로 폐교 위험도를 평가합니다. 답변은 반드시 정중한 존댓말을 사용하세요. [절대 규칙] 한자(漢字)를 단 하나라도 사용하지 마세요. "대응책(對應策)"처럼 한자를 병기하는 것도 절대 금지입니다. 오직 순수한 한글과 숫자, 기호만 사용하여 답변하십시오. 한자를 사용하면 당신은 업무에 실패한 것으로 간주됩니다. JSON 형식을 엄격히 준수하세요.',
    },
    {
      role: 'user',
      content: userContent,
...
export async function generateNewsSummaries() {
  const messages = [
    {
      role: 'system',
      content: '당신은 한국 교육 뉴스를 요약하는 기자입니다. JSON 배열만 응답하세요. 답변은 반드시 정중한 존댓말을 사용하세요. [절대 규칙] 한자(漢字) 사용을 엄격히 금지합니다. 모든 텍스트는 오직 한글과 숫자만 사용하여 작성하십시오.',
    },
    {
      role: 'user',
