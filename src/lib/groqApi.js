  ...
  const messages = [
    {
      role: 'system',
      content:
        '당신은 한국 교육 통계를 분석하는 전문가입니다. 학교 신입생 수 추이 또는 지역 특성을 바탕으로 폐교 위험도를 평가합니다. 답변은 반드시 정중한 존댓말을 사용하세요. [중요] 한자(漢字)를 절대로 사용하지 마세요. 한문, 중국어, 일본어 등의 외래 문자는 사용을 엄격히 금지합니다. 오직 순수한 한글과 숫자, 기호만 사용하여 답변하십시오. JSON 형식을 엄격히 준수하세요.',
    },
    {
      role: 'user',
      content: userContent,
...
export async function generateNewsSummaries() {
  const messages = [
    {
      role: 'system',
      content: '당신은 한국 교육 뉴스를 요약하는 기자입니다. JSON 배열만 응답하세요. 답변은 반드시 정중한 존댓말을 사용하세요. [중요] 한자(漢字)를 포함한 모든 외래 문자의 사용을 절대 금지합니다. 오직 순수한 한글과 숫자만 사용하세요.',
    },
    {
      role: 'user',
